...
def _load_data(path: str, class_names: list[str], debug_subset: Optional[int] = None):
    df = pr.read_csv(path)
...
def setup_datasets(config: wandb.Config, stage: str):
    ...
    if stage == "fit":
        df, comments, labels = _load_data(
            config.train_data, config.class_names, config.debug_subset
        )
        if len(config.class_names) == 2:
            stratifier = StratifiedShuffleSplit(
                n_splits=1, test_size=config.validation_split, random_state=config.seed
            )
        else:
        ...
        train_dataset = CommentDataset(
            comments[train_idx], labels[train_idx], tokenizer
        )
        val_dataset = CommentDataset(comments[val_idx], labels[val_idx], tokenizer)

        return df, train_dataset, val_dataset
    elif stage == "test":
        df, comments, labels = _load_data(
            config.evaluation_data, config.class_names, config.debug_subset
        )
